...
for op in [:similar, :copy, :unique]
    ...
    @eval (Base.$op)(x::GenomicRanges) =
        GenomicRanges(($op)(_genostarts(x)), ($op)(_genoends(x)), ($op)(_strands(x)), chr_info(x))
end

### Operations that take a GenomicVector and a Integer and return a GenomicVector
for op in [:similar, :resize!]
    @eval (Base.$op)(x::GenomicPositions, n::Integer) =
        GenomicPositions(($op)(_genostarts(x), n), chr_info(x))
    @eval (Base.$op)(x::GenomicRanges, n::Integer) = GenomicRanges(
        ($op)(_genostarts(x), n),
        ($op)(_genoends(x), n),
        ($op)(_strands(x), n),
        chr_info(x),
...
for op in [:issubset, :indexin, :findall] # max, min, etc. would go here if I decide that they make sense
    @eval begin
        function (Base.$op)(x::GenomicPositions, y::GenomicPositions, exact::Bool = true)
            same_genome(x, y) ||
                throw(ArgumentError("Both GenomicPositions must be from the same genome."))
...
for op in [:append!, :prepend!]
    @eval begin
        ...
        function (Base.$op)(x::GenomicRanges, y::GenomicRanges)
            same_genome(x, y) ||
                throw(ArgumentError("Both GenomicRanges must be from the same genome."))
