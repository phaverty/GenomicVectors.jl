...
@testset begin
    ...
    @test gr == GenomicRanges(
        [100, 40123, 300300, 500400],
        [120, 40456, 300350, 500455],
        [STRAND_NA, STRAND_POS, STRAND_NA, STRAND_NA],
        chrinfo,
    )
    ...
    sort!(gr)
    @test gr == gr2
    @test sort!(gr, rev = true) == GenomicRanges(
        ["chrX", "chr2", "chr2", "chr1"],
        [150, 300, 200, 400],
        [170, 320, 220, 420],
        ['+', '+', '-', '.'],
    ...
    e = s .+ 20
    gr = GenomicRanges(chrs, s, e, chrinfo)
    @test convert(DataFrame, gr) == DataFrame(
        [chrs, s, e, [STRAND_NA, STRAND_NA, STRAND_NA, STRAND_NA]],
        [:Chromosome, :Start, :End, :Strand],
    )
    @test convert(Vector{String}, gr) ==
          ["chr1:400-420", "chr2:300-320", "chr2:200-220", "chrX:150-170"]
    ...
    @test vcat(gr, gr) ==
          GenomicRanges(vcat(chrs, chrs), vcat(s, s), vcat(e, e), vcat(d, d), chrinfo)
